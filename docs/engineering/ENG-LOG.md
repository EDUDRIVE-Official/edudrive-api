...
## 2026-08-29 — IMP-078 (Cierre de ENG-078 — Gobierno de IA)

### Alcance acordado con el usuario

Primera historia planificada de la Fase 16 — Inteligencia artificial (ENG-077, Pagos y facturación, quedó marcada **Diferido** y se omitió en favor de esta). Única historia de todo el roadmap con una nota explícita de alineación documental: *"Debe alinearse con la documentación SEC y ARC correspondiente."* La investigación inicial no encontró ninguna documentación SEC/ARC dentro de este repositorio ni funcionalidad de IA/LLM existente en el código — se trató como un bloqueo genuino (no una ambigüedad resoluble por investigación) y se preguntó directamente al usuario en vez de adivinar el alcance o proceder a ciegas. El usuario indicó que los documentos viven en el repositorio hermano `D:\vr506\EDUDRIVE\edudrive-framework` y pidió que las respuestas fueran en español a partir de ese punto. Se localizaron y leyeron: **SEC-024 — Seguridad de Inteligencia Artificial y Modelos** (v0.1, borrador para revisión, 39 secciones) y **ARC-012 — Arquitectura de Inteligencia Artificial** (10 subdocumentos: Estrategia, Arquitectura de Modelos, Ciclo de Vida, MLOps, Integración, Gobierno, IA Responsable y Ética, Observabilidad, Seguridad y Privacidad, Roadmap). Con esa base documental se repropuso el alcance y el usuario eligió **alcance completo** (la segunda vez en toda la sesión, tras ENG-070, que se elige alcance completo por encima del reducido recomendado).

### Completado

- **`Modules\AiGovernance`** nuevo, con seis conceptos y un servicio transversal:
  - **`AiSystem`** (agregado): registro con nivel de riesgo (`AiRiskLevel` Ia0–Ia4), nivel de supervisión humana (`AiSupervisionLevel` entero Informs=1/Recommends=2/Proposes=3/Automates=4, ARC-012/07 §7), categorías de datos (`AiDataCategory`, incluye `Minors`) y máquina de estados `Evaluation→Pilot→Production⇄Suspended→Retired`. `promoteTo(Production)` aplica tres invariantes reales citando secciones concretas: IA-4 exige `extraordinaryApprovalGranted` (SEC-024 §4.5); IA-3 o IA-4 exigen `committeeApproved` (SEC-024 §28.2); datos `Minors` exigen supervisión `>= Proposes` (SEC-024 §8.4).
  - **`AiDecision`** (entidad, no agregado): primer flujo de revisión humana ("pendiente hasta que un humano apruebe o rechace") de todo el repositorio — deliberadamente separado de `Modules\Audit`, cuyo `AuditEntry` asume siempre un actor humano y no encaja con una decisión emitida por el propio sistema de IA. `requiresReview` se decide en la capa de Application/Infraestructura a partir del nivel de supervisión del `AiSystem`, no dentro de la propia entidad.
  - **`AiModel`**, **`AiPrompt`** (versión incremental en cada actualización de contenido, mismo patrón que `CommunicationTemplate`), **`AiIncident`**, **`AiProviderEvaluation`** — ciclos de vida propios, más simples.
  - **AI Gateway** (`Application\Services\AiGatewayClient` + `Infrastructure\Services\HttpAiGatewayClient`, ARC-012/03 §12 y ARC-012/06 §13, SEC-024 §11): invocación síncrona (a diferencia del disparo-y-olvido de Webhook/Mobile) contra un endpoint HTTP genérico configurable — no existe ni se planea ninguna integración real con un proveedor de IA en esta historia. Valida que el sistema esté en Piloto o Producción (`AiSystemNotUsable` si no), deriva `requiresReview` del `AiSupervisionLevel` real del sistema invocado y registra automáticamente una `AiDecision` real con tokens/costo/latencia incorporados como observabilidad (sin subsistema de observabilidad separado).
  - Los "20 usos prohibidos" de SEC-024 §25 no se modelaron como enum cerrado — la mayoría son políticas organizacionales sin punto de aplicación ejecutable en código. Se tradujo en su lugar la prohibición más concreta y verificable (IA-4 sin aprobación extraordinaria) en la invariante de dominio de `AiSystem` ya descrita.
  - Sin claves foráneas en los campos `*_owner_id`/`*_user_id` de gobernanza (a diferencia de `mobile_devices.user_id` en ENG-075) — son metadatos de gobernanza, no registros académicos ligados a identidad; decisión explícita para evitar el problema de integridad referencial en fixtures de prueba ya repetido en ENG-075 y ENG-076.
  - Migración con 6 tablas en orden de dependencia (`provider_evaluations` → `models` → `systems` → `decisions`/`prompts` → `incidents`), repositorios Eloquent estándar, 2 nuevos permisos (`ai_governance.manage`, `ai_governance.view`) otorgados únicamente a `Role::SuperAdmin`, ~35 rutas bajo `api/v1/ai-governance` con `auth:sanctum`, y un límite `ai-gateway` de 30 solicitudes/minuto por usuario para el endpoint de invocación.
  - **Corrección durante las pruebas Feature**: `AiGatewayController::invoke()` combinaba un parámetro `InvokeAiGatewayRequest` (FormRequest) con un segundo parámetro `Illuminate\Http\Request $httpRequest` — Laravel resuelve dependencias de método por tipo, y al ser `FormRequest` subclase de `Request`, el contenedor consideraba ya satisfecho el segundo parámetro con la misma instancia del primero, descolocando la inyección de `CommandBus` (error 500 en producción, no solo en pruebas). Corregido eliminando el parámetro `Request` separado y usando `$request->user()` directamente desde el FormRequest.

### Validaciones

- Suite completa de `Modules\AiGovernance` ✅ — `107 passed` (Domain 39, Integration/persistencia 18, Application 37, Feature 13).
- Pint ✅ sobre `modules/AiGovernance` (más `Authorization`, `Foundation`, `config`, `bootstrap` tocados); un `pint --test` sobre el repositorio completo solo encontró el mismo problema de estilo preexistente y no relacionado en `modules/Learning`, ya señalado y fuera de alcance en cierres previos.
- PHPStan (`--memory-limit=512M`) ✅ sin errores, tanto sobre `modules/AiGovernance modules/Authorization modules/Foundation` como sobre el repositorio completo.

**Estado:** Finalizado.

## 2026-08-29 — IMP-081 (Cierre de ENG-081 — Colas y trabajos asíncronos)

### Alcance acordado con el usuario

Primera historia de la Fase 17 — Plataforma y operación avanzada. El roadmap solo traía seis viñetas sueltas sin documento de diseño: Redis, Correos, Exportaciones, Procesamiento de archivos, Analítica, Reintentos. La investigación (dos agentes en background más lecturas dirigidas) encontró que Redis ya estaba completamente configurado (contenedor `redis:7-alpine` corriendo en `compose.yaml`, conexión `redis` completa en `config/queue.php`/`config/database.php`) pero sin usarse (`QUEUE_CONNECTION=database`), y que no existía ningún worker de cola — gap ya documentado explícitamente dos veces en el propio repo (cierres de ENG-062 y ENG-074) como la razón por la que ambas historias mantuvieron ciertas operaciones síncronas. Alcance propuesto: un tramo de infraestructura de colas (recomendado) o alcance completo agregando correo real, conversión de exportaciones/importaciones existentes a async, y un módulo de analítica nuevo. El usuario eligió **alcance completo** (la tercera vez en la sesión, tras ENG-070 y ENG-078). Detalle completo de las decisiones de diseño en `docs/plans/2026-08-29-colas-trabajos-asincronos-eng081-design.md`.

### Completado (en seis tramos, cada uno verificado y comprometido por separado)

- **Tramo A — Infraestructura de colas**: `QUEUE_CONNECTION=redis` activado en `.env`/`.env.example`; `REDIS_HOST` corregido de `127.0.0.1` (inalcanzable desde el contenedor `app`) a `redis` (nombre del servicio en `compose.yaml`); nuevo servicio `queue-worker` (`php artisan queue:work redis`), el primer worker de cola real del proyecto. `SendMobilePushJob` (único job sin configuración explícita) gana `$tries=3`/`backoff()`/`failed()`; `DeliverWebhookJob` no se toca — su `$tries=1` y reintento a nivel de dominio es una decisión ya cerrada de ENG-074.
- **Tramo B — `Modules\AsyncProcessing` (nuevo)**: primitivo genérico reutilizable en vez de un aggregate de seguimiento distinto por cada consumidor. `AsyncJob` (`type` string libre, `requestedByUserId` sin FK — mismo patrón de metadato-de-gobernanza que `Modules\AiGovernance`, `status` Pending/Processing/Completed/Failed, `result` JSON libre). `start()` es idempotente entre Pending y Processing (preserva la primera `startedAt`) para no chocar con el reintento nativo de Laravel tras un fallo a medio procesar. Autorización por propiedad, no por permiso: `GET /api/v1/async-jobs/{id}` verifica que `requestedByUserId` coincida con el solicitante, 404 si no. Validado como genuinamente reutilizable: lo consumen Academic, Admin, Identity y el nuevo Analytics sin que Foundation/AsyncProcessing conozca sus permisos específicos.
- **Tramo C — Exportaciones asíncronas**: `ExportAuditLogsHandler` (Admin), `ExportCoursesHandler`/`ExportEnrollmentsHandler` (Academic) pasan de generar el CSV en la misma request a crear un `AsyncJob` y despachar un Job propio que ejecuta la misma lógica CSV ya existente. Los tres endpoints devuelven 202 en vez de la URL inmediata. `ExportMyDataUseCase` (Identity) queda fuera a propósito — JSON de un solo usuario, no CSV administrativo.
- **Tramo D — Procesamiento de archivos asíncrono**: mismo patrón para los tres imports masivos existentes (`BulkImportUsersUseCase` en Identity, `BulkImportCoursesHandler`/`BulkImportQuestionsHandler` en Academic). El parsing del CSV subido sigue síncrono (ya acotado a 500 filas); el procesamiento fila-por-fila se mueve al Job. `CreateBulkEnrollmentsHandler`/`CreateBulkInstitutionalEnrollmentsHandler` quedan fuera a propósito — matrícula masiva, no parsean un archivo, ya cerrados en ENG-061/ENG-076.
- **Tramo E — Correo real**: `NotificationChannel::Email` era solo un valor de enum sin ninguna rama de efecto; Mailpit ya estaba provisionado en `compose.yaml` sin consumirse. Se agregó `NotificationMail` (Mailable) + `EmailNotificationSender`/`QueuedEmailNotificationSender`/`SendEmailNotificationJob` — mismo patrón puerto+implementación+job que `MobilePushSender` de ENG-075. `SendNotificationHandler` gana la rama `Email` análoga a la rama `Mobile`. `MAIL_MAILER=smtp`/`MAIL_HOST=mailpit` activado en `.env` real.
- **Tramo F — `Modules\Analytics` (nuevo, acotado)**: sin ningún detalle en el roadmap más allá de la palabra suelta, se interpretó como el snapshot más concreto y verificable posible — conteos reales agrupados por estado (matrículas, certificaciones, usuarios) calculados de forma asíncrona reutilizando el `AsyncJob` genérico, sin ningún componente de aprendizaje automático, deliberadamente para no invadir el territorio de ENG-080 (Análisis de patrones de conducción — riesgos, tendencias, explicabilidad — que permanece Diferido). `POST /api/v1/analytics/reports` (permiso nuevo `analytics.view`, solo `SuperAdmin`) dispara el cálculo; el resultado se consulta con el mismo endpoint genérico `GET /api/v1/async-jobs/{id}` del Tramo B, sin endpoint de consulta propio. `CertificateRepository` ganó el método `all()` (antes solo exponía `allForUser()`).

### Hallazgos durante las pruebas

- **Bug real corregido**: ningún controlador nuevo repitió el error de `AiGatewayController` de ENG-078 (FormRequest + `Request` separado) gracias a revisarlo explícitamente en cada nuevo endpoint.
- **Patrón de FK-en-fixtures recurrente** (ya visto en ENG-075/076/078): `audit_logs.user_id` y `certificates.course_id` tienen FK reales; los tests de los nuevos Jobs de importación y del reporte de certificaciones tuvieron que persistir un actor/curso real en vez de usar un UUID sintético.
- `AsyncJob::start()` tuvo que relajarse de "solo desde Pending" a "idempotente desde Pending o Processing" tras notar que el reintento nativo de Laravel (`$tries=3`) volvería a invocar `start()` sobre un Job que ya había pasado a `Processing` en el intento fallido anterior.

### Validaciones

- Suites dirigidas por tramo, todas en verde antes de continuar al siguiente: `AsyncProcessing` 18/18, exportaciones (Admin+Academic) 17/17, importaciones (Academic+Identity) 26/26, `Notification` 103/103, `Analytics` 9/9, `Certification`+`Authorization` 129/129.
- Suite completa de `Modules\Academic` (877 passed, 1 failed) y `Modules\Admin` (48 passed) con `memory_limit=512M` — el único fallo (`GradingResultTest.php:99`, `TypeError` vs `InvalidArgumentException`) es preexistente y no relacionado con esta historia.
- Pint ✅ sobre todos los módulos tocados; un `pint --test` sobre el repositorio completo solo encontró el mismo problema de estilo preexistente en `modules/Learning` ya señalado en cierres previos.
