...
it('envia una notificacion nueva a un usuario', function (): void {
    ...
    $response = (new SendNotificationHandler($notifications))->handle(new SendNotificationCommand(
        userId: $userId,
        channel: 'web',
        category: 'logro',
...
it('marca como leida una notificacion propia', function (): void {
    $notifications = new InMemoryNotificationRepository;
    $userId = (string) Str::uuid();
    $sent = (new SendNotificationHandler($notifications))->handle(new SendNotificationCommand($userId, 'web', 'logro', 'Asunto', 'Cuerpo'));
...
it('rechaza marcar como leida una notificacion de otro usuario', function (): void {
    $notifications = new InMemoryNotificationRepository;
    $sent = (new SendNotificationHandler($notifications))->handle(new SendNotificationCommand((string) Str::uuid(), 'web', 'logro', 'Asunto', 'Cuerpo'));
...
it('lista las notificaciones del usuario autenticado', function (): void {
    $notifications = new InMemoryNotificationRepository;
    $userId = (string) Str::uuid();
    (new SendNotificationHandler($notifications))->handle(new SendNotificationCommand($userId, 'web', 'logro', 'Asunto 1', 'Cuerpo 1'));
    (new SendNotificationHandler($notifications))->handle(new SendNotificationCommand($userId, 'email', 'certificado', 'Asunto 2', 'Cuerpo 2'));
    (new SendNotificationHandler($notifications))->handle(new SendNotificationCommand((string) Str::uuid(), 'web', 'logro', 'De otro usuario', 'Cuerpo'));
